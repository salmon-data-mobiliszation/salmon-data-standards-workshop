...
2. `episodes/session-2.Rmd`: create a draft package.
3. `episodes/session-3.Rmd`: capture context.
4. `episodes/session-4.Rmd`: review terms and measurement meanings.
5. `episodes/session-5.Rmd`: code lists, SKOS, and local vocabulary.
6. `episodes/session-6.Rmd`: route term gaps and plan publication.
